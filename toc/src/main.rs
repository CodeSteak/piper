...
use common::{TarPassword, TarHash};
use config::Config;
use std::{path::{PathBuf}, str::FromStr, fs::Permissions, os::unix::prelude::PermissionsExt, io::{Read, Write}, fmt::Display};
use anyhow::Context;
...
fn tar_password_parser(input : &str) -> Result<TarUrl, String> {
    ...
    let code = TarPassword::from_str(input.trim_end_matches("/")).map_err(|_| {
        format!("Invalid code: {}.", input)
    })?;
...
fn main() -> anyhow::Result<()> {
    ...
    cli.protocol = cli.protocol.or_else(|| config.protocol.clone());
    ...
    match cli.subcmd {
        ...
        Some(Commands::Login) => {
            let file = Config {
                ...
                history_file : if cli.no_history_file {None} else {cli.history_file},
            }.save(&cli.config)?;
...
fn receive(cli : &Cli) -> anyhow::Result<()> {
    ...
    let host = code.host.as_ref().or(cli.host.as_ref()).ok_or_else(|| {
        anyhow::anyhow!("No host specified.")
    })?;
    let protocol = code.protocol.or(cli.protocol).unwrap_or(config::Protocol::Https);
    ...
    let content_length = response.header("Content-Length").and_then(|s| s.parse::<u64>().ok()).unwrap_or(0);
    ...
    for entry in tar.entries()? {
        ...
        if file.header().entry_type().is_dir() {
            ...
            let mut new_file = if overwrite {
                std::fs::OpenOptions::new().write(true).create(true).truncate(true).open(&file_destination)
            } else {
                std::fs::OpenOptions::new().write(true).create_new(true).open(&file_destination)
...
impl ProgressBar {
    ...
    fn update<D : Display>(&mut self, progress : u64, message : D) {
        ...
        let percent = if self.current < self.total && self.total > 0 {(self.current as f64 / self.total as f64) * 100.0} else {100.0};
        ...
        let bar = (0..((percent / 5.0) as isize)).map(|_| "=").collect::<String>();
...
const DELETE_LINE : &'static str = "\x1B[2K\r";
